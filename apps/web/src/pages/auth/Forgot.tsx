...
export function Forgot() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (error) {
        toastError(error.message || t('auth.forgot.errorTitle'), t('auth.forgot.errorDesc'))
      } else {
        toastSuccess(t('auth.forgot.successTitle'), t('auth.forgot.successDesc'))
      }
    } catch (err: any) {
      toastError(t('auth.forgot.errorTitle'), err.message)
    } finally {
      setLoading(false)
    }
  }
      ...
      >
        <div>
          ...
          <form onSubmit={handleSubmit} className="space-y-3">
            <input
              type="email" required placeholder={t('auth.forgot.emailPlaceholder')}
