          ...
          <div className="grid grid-cols-2 gap-1">
            {(['top', 'iso', 'corridor', 'free'] as ViewMode[]).map(v => (
              <button
                ...
                className={`py-1.5 text-[10px] font-medium rounded-md border transition-colors ${viewMode === v
          ...
          <div className="flex flex-wrap gap-1">
            {Array.from({ length: cfg.floors }, (_, i) => (
              <button
                ...
                className={`px-2.5 py-1 text-[10px] font-medium rounded border transition-colors ${i === currentFloor
                    ? 'bg-primary text-white border-primary'
                    : 'bg-background text-muted-foreground border-border hover:bg-muted'
                  }`}
              >
              ...
              <div className="flex gap-1">
                {(['top', 'iso', 'corridor', 'free'] as ViewMode[]).map(v => (
                  <button
                    key={v}
                    onClick={() => handleSetView(v)}
                    className={`px-2 py-1 text-[10px] font-medium rounded-md border shadow-sm transition-colors backdrop-blur-sm ${viewMode === v
                        ? 'bg-primary text-white border-primary'
              ...
              <div className="flex flex-wrap-reverse justify-end gap-1 max-w-[160px]">
                {Array.from({ length: cfg.floors }, (_, i) => (
                  <button
                    key={i}
                    onClick={() => handleFloorSelect(i)}
                    className={`w-8 h-7 text-[10px] font-semibold rounded-md border shadow-sm transition-colors backdrop-blur-sm ${i === currentFloor
                        ? 'bg-primary text-white border-primary'
                        : 'bg-white/90 text-muted-foreground border-border/70 hover:bg-white hover:text-foreground'
                      }`}
                  >
